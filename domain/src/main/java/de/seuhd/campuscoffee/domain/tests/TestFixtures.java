...
public class TestFixtures {
    ...
    private static final List<User> USER_LIST = List.of(
            User.builder()
                    .id(1L).creationTimestamp(DATE_TIME).updateTimestamp(DATE_TIME)
                    .login("jane_doe")
                    .email("jane.doe@uni-heidelberg.de")
                    .firstName("Jane")
                    .lastName("Doe")
                    .build(),
            User.builder()
                    .id(2L).creationTimestamp(DATE_TIME).updateTimestamp(DATE_TIME)
                    .login("maxmustermann")
                    .email("max.mustermann@campus.de")
                    .firstName("Max")
                    .lastName("Mustermann")
                    .build(),
            User.builder()
                    .id(3L).creationTimestamp(DATE_TIME).updateTimestamp(DATE_TIME)
                    .login("student2023")
                    .email("student2023@study.org")
                    .firstName("Student")
                    .lastName("Example")
                    .build()
    );
    ...
    public static List<User> getUserListForInsertion() {
        return getUserList().stream()
                .map(user -> user.toBuilder().id(null).creationTimestamp(null).updateTimestamp(null).build())
                .toList();
    }
